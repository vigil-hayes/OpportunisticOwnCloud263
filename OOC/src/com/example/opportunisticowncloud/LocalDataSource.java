...
public class LocalDataSource {
	...
	// Database fields
	  ...
	  public LocalRemoteEntry createLocalToRemoteEntry(String name, long time) {
		    ContentValues values = new ContentValues();
		    LocalRemoteEntry newLREntry = new LocalRemoteEntry();
		    values.put(LocalOpenHelper.COLUMN_NAME, name);
		    values.put(LocalOpenHelper.COLUMN_MODIFIED, time);
		    try {
	  ...
	  public List<LocalRemoteEntry> getAllLocalRemoteEntries() {
		    List<LocalRemoteEntry> lrelist = new ArrayList<LocalRemoteEntry>();
		    try {
		    	Cursor cursor = database.query(LocalOpenHelper.TABLE_LOCAL,
		        allColumns, null, null, null, null, null);
		    	cursor.moveToFirst();
			    while (!cursor.isAfterLast()) {
			      LocalRemoteEntry lre = cursorToLocalRemoteEntry(cursor);
			      lrelist.add(lre);
			      cursor.moveToNext();
			    }
